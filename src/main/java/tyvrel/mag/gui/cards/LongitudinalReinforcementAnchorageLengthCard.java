...
import tyvrel.mag.core.factory.longitudinalreinforcement.LongitudinalReinforcementDesignAnchorageLengthFactory;
 ...
 */
public class LongitudinalReinforcementAnchorageLengthCard extends BaseJPanel {
	...
	public LongitudinalReinforcementAnchorageLengthCard(String name) {
		super(name);
		...
		jButton.addActionListener(e -> {
			try {
				jLabel.setText(
						"Obliczona długość zakotwienia wynosi: " + String.format("%.0f", calculateAnchorageLength() *
								1000) + "mm");
	...
	public double calculateAnchorageLength() throws ImproperDataException, LSException {
		double lbrqd = new LongitudinalReinforcementRequiredAnchorageLengthFactory(
				...
				.build();
		return new LongitudinalReinforcementDesignAnchorageLengthFactory(
				reinforcementDiameterPanel.getDiameters()[0],
				lbrqd
		).build();
	}
